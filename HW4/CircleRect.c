...
int main(int argc, char* argv[]){
  ...
  Point2 C;
  double Rad;
  R.max.x = atof(argv[1]); R.max.y = atof(argv[2]);
  R.min.x = atof(argv[3]); R.min.y = atof(argv[4]);
  C.x = atof(argv[5]); C.y = atof(argv[6]);
  Rad = atof(argv[7]);
  //printf("%f %f %f %f %f %f %f\n",R.max.x, R.max.y, R.min.x, R.min.y, C.x, C.y, Rad);
  //intersect = Check_Intersect(&R, &C, Rad);
